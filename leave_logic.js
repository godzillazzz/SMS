...
window.impersonateUser = function(userId) {
  ...
  const currentSession = JSON.parse(localStorage.getItem('shiftflow_session_v1') || sessionStorage.getItem('shiftflow_session_v1') || '{}');
  if (!currentSession || (!currentSession.Email && !currentSession.EmployeeID)) {
    alert('ไม่พบเซสชั่น Admin ของคุณ กรุณาเข้าสู่ระบบอีกครั้ง');
    return;
  }
  if (!localStorage.getItem('shiftflow_original_admin_v1')) {
    localStorage.setItem('shiftflow_original_admin_v1', JSON.stringify(currentSession));
  }
  ...
  const mockSession = {
    EmployeeID: targetUser['User ID'] || targetUser.EmployeeID || userId,
    Name: targetUser.Name || targetUser.FullName || ('พนักงานรหัส ' + userId),
    FullName: targetUser.FullName || targetUser.Name || ('พนักงานรหัส ' + userId),
    Email: targetUser.Email || (userId + '@ptt.com'),
    Role: targetUser.Role || 'Viewer',
    Department: targetUser.Department || 'ส่วนปฏิบัติการระบบท่อเขต 11',
    Zone: targetUser.Zone || 'เขต 11',
    Status: 'Active',
    isImpersonated: true,
    originalAdminEmail: currentSession.Email || currentSession.email || 'Admin'
  };
  
  localStorage.setItem('shiftflow_session_v1', JSON.stringify(mockSession));
  sessionStorage.setItem('shiftflow_session_v1', JSON.stringify(mockSession));
  localStorage.setItem('shiftflow_is_impersonating', 'true');
  
  alert('🐞 โหมด View As User: เข้าสู่ระบบในมุมมองของ ' + mockSession.Name + ' (Role: ' + mockSession.Role + ') เรียบร้อยแล้ว!');
  location.reload();
};

window.exitImpersonation = function() {
  if (!confirm('⬅️ ยืนยันกลับสู่บัญชี Admin ของคุณใช่หรือไม่?')) return;
  const originalAdminStr = localStorage.getItem('shiftflow_original_admin_v1');
  if (originalAdminStr) {
    ...
    localStorage.removeItem('shiftflow_session_v1');
    sessionStorage.removeItem('shiftflow_session_v1');
    localStorage.removeItem('shiftflow_is_impersonating');
    location.reload();
  }
};
...
window.checkImpersonationStatus = function() {
  const originalAdminStr = localStorage.getItem('shiftflow_original_admin_v1');
  const isImpersonating = localStorage.getItem('shiftflow_is_impersonating') === 'true';
  if (originalAdminStr && isImpersonating) {
    const currentSession = JSON.parse(localStorage.getItem('shiftflow_session_v1') || sessionStorage.getItem('shiftflow_session_v1') || '{}');
    ...
    if (!banner) {
      ...
      banner.innerHTML = `
        <span>🚨 <b>โหมด Debug (View As):</b> คุณกำลังใช้งานในมุมมองของ <u>${currentSession.Name || 'พนักงาน'}</u> (Role: ${currentSession.Role || 'Viewer'})</span>
